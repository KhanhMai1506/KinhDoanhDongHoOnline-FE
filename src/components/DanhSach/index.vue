          ...
          <div class="col-lg-5 text-lg-end">
            <div class="row row-cols-lg-1 g-2">
              <div class="col">
                <button
                  type="button"
                  class="btn btn-outline-secondary me-2 px-3 radius-30"
                  @click="sortType = 'moi'"
                >
                  Mới Nhất
                </button>
    ...
    filteredProducts() {
      return this.list_san_pham.filter((value) => {
        ...
        if (this.priceFilter) {
          ...
          matchesPrice = value.gia_ban >= min && value.gia_ban <= max;
        }

        return matchesSearch && matchesPrice;
      }).sort((a, b) => {
        if (this.sortType === "moi") return b.id - a.id;
        if (this.sortType === "tang") return a.gia_ban - b.gia_ban;
        if (this.sortType === "giam") return b.gia_ban - a.gia_ban;
        return 0;
      });
    },
    paginatedProducts() {
